...
namespace PetaPoco
{
    ...
    public class Database : IDisposable
    {
        ...
        //
        public DateTime OnExecutingTime
        {
            get;
            set;
        }
        public DateTime OnExecutedTime
        {
            get;
            set;
        }
            ...
            try
            {
                ...
                try
                {
                    using (var cmd = CreateCommand(_sharedConnection, sql, args))
                    {
                        ...
                        if (val == null) { val = 0; } //yiyioo add ,暂时先加一个默认值，防止val==null的时候出exception
                        OnExecutedCommand(cmd);
        ...
        static Regex rxGroup = new Regex(@"\bGROUP\s+BY\s+", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Singleline | RegexOptions.Compiled);
        static Regex rxWhere = new Regex(@"\bWHERE\s+", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Singleline | RegexOptions.Compiled);
        //static Regex rxWheres = new Regex(@"\bWHERE\s+((?:\((?>\((?<depth>)|\)(?<-depth>)|.?)*(?(depth)(?!))\)|.)*?)(?<!,\s+)", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Singleline | RegexOptions.Compiled);
        public static bool SplitSqlForPaging(string sql, out string sqlCount, out string sqlSelectRemoved, out string sqlOrderBy)
        {
            ...
            var m3 = rxWhere.Split(sql);
            //var m2 = rxWhere.Match(sql);
            //var sqlWhere = sql.Substring(m2.Index);
            var sqlWhere = m3.Last();
            //
            ...
            //todo:对于复杂的嵌套sql，这个可能会误判。
            if (rxGroup.IsMatch(sqlWhere))
            {
                sqlCount = String.Format("SELECT COUNT(0) FROM ({0})", sqlCount);

                //sqlCount = "SELECT COUNT(*) FROM (" + sql.Substring(0, g.Index) + m.Groups[1].ToString().Trim() + sql.Substring(g.Index + g.Length) + ")";
            }
            ...
            //    if (rxGroup.IsMatch(sqlWhere))
            //    {
            //        sqlCount = "SELECT COUNT(*) FROM (" + sql.Substring(0, g.Index) + " * " + sql.Substring(g.Index + g.Length) + ")";
            //    }
            //    else
            //    {
            //        sqlCount = sql.Substring(0, g.Index) + "COUNT(*) " + sql.Substring(g.Index + g.Length);
                        ...
                        if (columns == null)
                        {
                            foreach (var i in pd.Columns)
                            {
                                ...
                                if (i.Value.ResultColumn)
                                    continue;
                                //
                                if (i.Value.PropertyInfo.GetCustomAttributes(typeof(NotUpdateAttribute), true).Length != 0)
                                    continue;
                                // Build the sql
                                if (index > 0)
                                    sb.Append(", ");
